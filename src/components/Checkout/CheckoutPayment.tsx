...
export async function CheckoutPayment(props: CheckoutPageProps) {
  ...
  const sp = getServerSearchParams();
 	const targetOrder = await Order.get_single(
    object_get(sp, 'id', 'order', '_i')
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <b className="text-justify text-xs leading-6 p-5" dir="rtl">
        برای ادامه فرایند جمع مبلغ خرید شده را به شماره حسابی که در زیر درج شده
        است واریز نمایید سپس شماره پیگیری واریزی را در قسمت زیر وارد کنید یا به
        شماره{' '}
        <bdi>
      ...
      >
        ...
        <button className="btn w-full max-w-xs">ارسال</button>
      </form>
    </div>
  );
}
