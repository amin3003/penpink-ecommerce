...
export default async function ProductList(props: ProductListProps) {
  ...
  return (
    <div className=" w-full grid xl:grid-cols-4 xl:gap-4 lg:grid-cols-3 lg:gap-6 md:grid-cols-3 grid-cols-1 gap-4 mx-auto mt-3">
      {data.map((r: any, index: any) => {
        return <ProductCard className={"mx-auto"} key={index} product={r} />;
      })}
    </div>
  );
}
